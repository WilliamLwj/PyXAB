...
def test_binary_partition_1D_make_children():
    domain = [[0, 1]]
...
def test_binary_partition_1D_deepen():
    ...
    for i in range(5):
        ...
        for j in range(len(nodelist[-1])):
            assert_allclose(nodelist[-1][j].get_domain(), [[j/(2**(i+1)), (j+1)/(2**(i+1))]])
    ...
    for depth in range(part.get_depth() - 1):
        for parent in nodelist[depth]:
            ...
            for i in range(len(domain)):
                if parent_domain[i][1] != child_domain_1[i][1] and parent_domain[i][0] != child_domain_2[i][0]:
                    ground_truth_domain_1 = copy.deepcopy(parent_domain)
                    ground_truth_domain_1[i][0] = parent_domain[i][0]
                    ground_truth_domain_1[i][1] = (parent_domain[i][0] + parent_domain[i][1])/2

                    assert_allclose(child_domain_1, ground_truth_domain_1)

                    ground_truth_domain_2 = copy.deepcopy(parent_domain)
                    ground_truth_domain_2[i][0] = (parent_domain[i][0] + parent_domain[i][1])/2
